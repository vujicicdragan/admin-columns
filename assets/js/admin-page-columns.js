...
if(boxes.hasClass('disabled')){boxes.find('.ac-column').each(function(i,col){$(col).column_bind_toggle();$(col).find('input, select').prop('disabled',true)})}else{var _columns=boxes.find('.ac-columns');// we start by binding the toggle and remove events.
_columns.find('.ac-column').each(function(i,col){$(col).column_bind_toggle();$(col).column_bind_remove();$(col).column_bind_clone();$(col).cpac_bind_indicator_events();$(col).column_onload()});// ordering of columns
_columns.cpac_bind_ordering()}// hook for addons
...
$(column).find('.ac-column-setting--filter').cpac_column_sub_setting_toggle();$(column).find('.ac-column-setting--sort').cpac_column_sub_setting_toggle();$(column).find('.ac-column-setting--edit').cpac_column_sub_setting_toggle();$('.ac-setting-input_type').select2({width:'100%',templateResult:function templateResult(state){if(!state.id){return state.text}state.text=state.text.replace('(PRO)','<span  class="ac-type-pro-only">pro</span>');return $('<span>'+state.text+' </span>')},templateSelection:function templateSelection(data){var $value=$('<div>'+data.text+'</div>');$value.find('.ac-type-pro-only').remove();return $value.text()}})});// AC Modal Events (todo move to separate logic)
$().ready(function(){$(document).on('click','[data-ac-open-modal]',function(e){e.preventDefault();$($(this).data('ac-open-modal')).addClass('-active')});$('.ac-modal__dialog__close').on('click',function(e){e.preventDefault();$(this).closest('.ac-modal').removeClass('-active')});$('.ac-modal').on('click',function(){$(this).removeClass('-active')});// Prevent bubbling
